...
from unet_models import UNET, NEST_UNET
...
class SuperResolution:
    def __init__(self, sr_method, channels):
        ...
        data = np.load('./DATA/Training_1959-2015_Scaled.npz')
        if self.channels == 'RF':
    ...
    def Model_Training(self, expid, epochs, learning_rate, decay_rate, batch_size, model_save_path, csv_logger_path, restart, initial_epoch):
        ...
        if restart == True:
            ...
            if self.sr_method == 'SRCNN':
                ...
                print(model.summary())
            elif self.sr_method == 'UNET':
                model = UNET(layer_N=[64, 96, 128, 160], input_size=(32,32,3), input_stack_num=2, pool=True, activation='prelu')
                print(model.summary())
            elif self.sr_method == 'NEST_UNET':
                model = NEST_UNET(layer_N=[32, 64, 128, 256], input_size=(32,32,3), input_stack_num=2, pool=True, activation='prelu')
                print(model.summary())
